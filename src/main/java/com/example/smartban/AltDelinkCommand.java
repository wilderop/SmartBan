...
public class AltDelinkCommand implements CommandExecutor {

    private final SmartBanPlugin plugin;

    public AltDelinkCommand(SmartBanPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length != 2) {
            sender.sendMessage("Usage: /altdelink <player1> <player2>");
            return true;
        }
        ...
        plugin.getAltManager().delinkAlts(uuid1, uuid2);
        sender.sendMessage("Delinked " + args[0] + " and " + args[1] + ".");
        return true;
    }
}
